...
<template>
    ...
    <el-form ref="formRef" size="large" class="login-content-form" :model="state.ruleForm" :rules="rules" @keyup.enter="loginClick">
      ...
      <el-form-item class="login-content-remember login-animation3">
        <div style="display: flex; justify-content: space-around; align-items: center; width: 100%;">
          <el-checkbox
              v-model="ruleForm.remember"
              :style="{ '--el-color-primary': '#EE742A' }"
          >
            {{ $t('message.account.rememberPassword') }}
          </el-checkbox>

          <a
              class="forgot-password"
              @click.prevent="handleForgotPassword"
              style="color: #EE742A; cursor: pointer; font-size: 14px;"
          >
            忘记密码？
          </a>
        </div>
      ...
      <div style="text-align: center" class="login-animation4">
        <span style="color: #7a7a7a">{{ $t('message.account.versionInfo') }}</span>
      </div>
    </el-form>
  </div>
  <!--      申请试用-->
...
.form-wrapper {
  ...
  transform-origin: top center; /* 控制放大的参考点 */
}
.forgot-password {
  color: #EE742A;
  cursor: pointer;
...
.login-content-form {
  ...
  .login-content-captcha {
    ...
    font-weight: bold;
    letter-spacing: 5px;
    margin-bottom: 10px;  // 调整验证码与下面元素的间距
  }

  /* 记住密码复选框的样式 */
  .login-content-remember {
    margin-top: -20px;  // 调整“记住密码”复选框与其他元素的间距
    margin-bottom: -10px;  // 调整复选框和登录按钮之间的间距
    margin-left: 5px;
  }
  ...
  .login-content-submit {
    ...
    letter-spacing: 2px;
    font-weight: 800;
    margin-top: 10px;  // 减少登录按钮顶部的间距
    background: linear-gradient(to right, #E7DFD7, rgb(255, 140, 26)); /* 渐变背景 */
    border: 0px solid #979797; /* 边框颜色 */
    color: #ffffff; /* 文字颜色 */
    transition: all 0.3s ease; /* 平滑过渡 */
  }

  .login-content-submit:hover {
    filter: brightness(1.08); /* 整体变亮 */
    cursor: pointer;
  }
}
